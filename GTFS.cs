...
namespace GTFS
{
    public class GTFS
    {
        private void LoadTable(DataTable table, ZipArchiveEntry entry)
        {
        ...
        public List<RouteMatrixCalendar> RouteMatrix
        {
            get
            {
                ...
                {
                    var thisCalendar = new RouteMatrixCalendar { calendar_txt = calendar_txtRow, Routes = new List<RouteMatrixRoute>() };
                    calendarMatrix.Add(thisCalendar);
                    foreach (var route_textRow in GTFSset._routes_txt.Rows.Cast<GTFSset._routes_txtRow>())
                    {
                        var thisRoute = new RouteMatrixRoute { route_short_name = route_textRow.route_short_name, route_txt = route_textRow, Directions = new List<RouteMatrixDirection>() };
                        thisCalendar.Routes.Add(thisRoute);
                        foreach (var direction_id in new String[] { "0", "1" })
                        {
                            var thisDirection = new RouteMatrixDirection { direction_id = direction_id, Stops = new LinkedList<RouteMatrixStop>()};
